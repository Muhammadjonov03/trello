...
<li className={style.boardTask}>
  <div className={style.attachmentsWrap}>
    {task.attachments && task.attachments.map(attachment =><span className={style.attachment + ' ' + (attachmentActive
      && style.attachmentActive)} style={{backgroundColor: attachment.color.color}} onClick={onAttClick}>
      {attachment.title}
    </span>)}
  </div>
